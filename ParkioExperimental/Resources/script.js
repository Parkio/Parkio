...
function getParkingSpots(pos, callback) { //pos is the center position, callback is the function the data will be passed into
    ...
    ], callback); //Send position to the server
}
...
function getParkingSpaceImage(timeHours, parkingSpace) {
    if (timeHours >= parkingSpace['end'] || timeHours < parkingSpace['start']) { //If time is past end or before start
        return {
            url: "http://s23.postimg.org/txyppobh3/red.png",
            scaledSize: new google.maps.Size(20, 20)
        }
    } else if (timeHours + 1 >= parkingSpace['end']) { //If time is about to expire
        return {
            url: "http://s27.postimg.org/owrgzc1hr/yellow.png",
            scaledSize: new google.maps.Size(20, 20)
        }
    } else { //Otherwise fine.
        return {
            url: "http://s27.postimg.org/6fsgurw5r/green.png",
            scaledSize: new google.maps.Size(20, 20)
        }
    }
}
...
function buildMarkers(parkingSpaces) {
    //Build markers
    var d = new Date();
    var timeHours = d.getHours() + d.getMinutes()/60;
